...
[![Tests: 258 passed](https://img.shields.io/badge/tests-258%20passed-brightgreen.svg?logo=pytest&logoColor=white)](https://docs.pytest.org/)
...
├──────────────────────────────┬───────────────────────────────────┬───────────────────────────────┤
│       PORTSIDE / LEFT        │      CENTER (ДП / ШТУРВАЛ)        │       STARBOARD / RIGHT       │
│                              │                                   │                               │
...
│    [🔒 CLAMPED] [-5%] [+5%]  │     [AXIOMETER: ACTUAL vs CMD]    │    [🔒 CLAMPED] [-5%] [+5%]   │
│                              │     [ ДП: 0° / 2x Клик Сброс ]    │                               │
│ 🔹 FURLING LINE              ├───────────────────────────────────┤ 🔹 MAIN HALYARD (ФАЛ)         │
│    [🔒 CLAMPED] [-5%] [+5%]  │ ⚓ ПОГОН ГИКА (TRAVELER) [🔒 BRAKE]│    [🔒 CLAMPED] [-5%] [+5%]   │
│                              │    [◄ PORT (-100%) ... (+100%) ►] │                               │
│ 🔹 CUNNINGHAM                ├───────────────────────────────────┤ 🔹 OUTHAUL                    │
│    [🔒 CLAMPED] [-5%] [+5%]  │ ⚓ РИФЛЕНИЕ ГРОТА (REEF PRESETS)   │    [🔒 CLAMPED] [-5%] [+5%]   │
│                              │    [FULL MAIN] [REEF 1] [REEF 2]  │                               │
│ 🔹 REEF LINE 1               ├───────────────────────────────────┤ 🔹 BOOM VANG (ОТТЯЖКА ГИКА)   │
│    [🔒 CLAMPED] [-5%] [+5%]  │ 🔹 MAINSHEET (ГРОТА-ШКОТ)         │    [🔒 CLAMPED] [-5%] [+5%]   │
│                              │    TRIM: [██████░░] 75%           │                               │
│ 🔹 REEF LINE 2               │    LOAD: 1250 N / SWL 4000 N      │ 🔹 INTERLOCK SAFETY ALERT     │
│    [🔒 CLAMPED] [-5%] [+5%]  │    [🔒 CLAMPED] [-5%] [+5%]       │    Status: HARDWARE GUARD OK  │
└──────────────────────────────┴───────────────────────────────────┴───────────────────────────────┘
```
...
2. **Интерактивные карточки 10 канатов и фалов**:
   ...
   - Интерактивное плавное натяжение мышью (**Drag & Drop**) и колесом мыши прямо над треком, а также дискретные кнопки точной подстройки $\pm 5\%$.
...
Проект покрыт **258 автоматизированными тестами**:
...
* `test_workbench_server.py` — Верификация эндпоинтов `/api/scenarios`, `/api/run`, `/api/sails`, `/api/query-action`, `/api/v1/telemetry/rig`, `/api/v1/controls/rig`.
...
# Запуск полного набора тестов (258 passed)
uv run pytest
```
