...
[![Tests: 263 passed](https://img.shields.io/badge/tests-263%20passed-brightgreen.svg?logo=pytest&logoColor=white)](https://docs.pytest.org/)
...
- [🎛 Интерактивный пульт шкипера и контроль такелажа (Rig & Cockpit Console)](#-интерактивный-пульт-шкипера-и-контроль-такелажа-rig--cockpit-console)
- [⚓ 3-Уровневая модель управления парусами (Single Source of Truth)](#-3-уровневая-модель-управления-парусами-single-source-of-truth)
- [🗗 Автономный мостик на отдельном мониторе (Standalone Pop-out Bridge)](#-автономный-мостик-на-отдельном-мониторе-standalone-pop-out-bridge)
...
5. **Единый источник истины для управления рангоутом и парусами (Single Source of Truth)**:
   - Все оперативные манипуляции парусами, точками крепления (трэками) и рифами сосредоточены в **Кокпите**, исключая дублирующие интерфейсы и конфликты состояний.
...
## 🎛 Интерактивный пульт шкипера и контроль такелажа (Rig & Cockpit Console)

В симулятор интегрирован 3-колоночный интерактивный пульт прямого управления вооружением яхты и рулевым устройством (**`CockpitController`**):

```
┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
│ ⚙ COCKPIT   [🕹 ШКИПЕР (РУЧНОЕ)] [🗗 В окне]                 AWA: 040° | AWS: 15.0 kt | STBD TACK│
├──────────────────────────────────────────────────────────────────────────────────────────────────┤
│ ⚓ [Active Sail Plan]  [Main: MAINSAIL R1 75%]  [Inner: —]  [Outer: GENOA]  [Bow: —]             │
├──────────────────────────────┬───────────────────────────────────┬───────────────────────────────┤
│           PORTSIDE           │      CENTER (ДП / ШТУРВАЛ)        │           STARBOARD           │
│                              │                                   │                               │
│ 🔹 JIB SHEET PORT            │ ⚓ ШТУРВАЛ И ПЕРО РУЛЯ [HYDRO 100%] │ 🔹 JIB SHEET STARBOARD        │
│    TRIM: [████░░░░] 50%      │          ( ⎈ 186px SVG )          │    TRIM: [░░░░░░░░] 0% (SLACK)│
│    LOAD: 450 N / SWL 2500 N  │     [HELM: +15.0° | 0.25 об]      │    LOAD: 25 N / SWL 2500 N    │
│    [🔒 CLAMPED]               │     [AXIOMETER: ACTUAL vs CMD]    │    [🔒 CLAMPED]               │
│                              │     [ ДП: 0° / 2x Клик Сброс ]    │                               │
│ 🔹 FURLING LINE              ├───────────────────────────────────┤ 🔹 MAIN HALYARD (ФАЛ)         │
│    [🔒 CLAMPED]               │ ⚓ ПОГОН ГИКА (TRAVELER) [🔒 BRAKE]│    [🔒 CLAMPED]               │
│                              │    [◄ PORT (-100%) ... (+100%) ►] │                               │
│ 🔹 CUNNINGHAM                ├───────────────────────────────────┤ 🔹 OUTHAUL                    │
│    [🔒 CLAMPED]               │ ⚓ РИФЛЕНИЕ ГРОТА (REEF PRESETS)   │    [🔒 CLAMPED]               │
│                              │    [FULL MAIN] [R1] [R2] [R3]     │                               │
│ 🔹 REEF LINE 1               ├───────────────────────────────────┤ 🔹 BOOM VANG (ОТТЯЖКА ГИКА)   │
│    [🔒 CLAMPED]               │ 🔹 MAINSHEET (ГРОТА-ШКОТ)         │    [🔒 CLAMPED]               │
│                              │    TRIM: [██████░░] 75%           │                               │
│ 🔹 REEF LINE 2 / 3           │    LOAD: 1250 N / SWL 4000 N      │ 🔹 INTERLOCK SAFETY ALERT     │
│    [🔒 CLAMPED]               │    [🔒 CLAMPED]                   │    Status: HARDWARE GUARD OK  │
└──────────────────────────────┴───────────────────────────────────┴───────────────────────────────┘
```

---

## ⚓ 3-Уровневая модель управления парусами (Single Source of Truth)

Вся логика работы с парусами разделена на 3 строгих уровня:

1. **L1: Паспорт яхты и гардероб рундуков (Vessel Specs & Locker Wardrobe)**:
   - Блок 3 слева отображает неизменные параметры корпуса (`LOA`, `Beam`, `Mass`, `Base Sail Area`).
   - Модальное окно `Custom Vessel` позволяет редактировать геометрию судна и отмечать флажками паруса, имеющиеся на борту в рундуках.
2. **L2: Тактический план поднятых парусов (Active Sail Plan)**:
   - Горизонтальная плашка в шапке кокпита отображает поднятые в данный момент паруса по 4 точкам крепления: `Main Mast`, `Inner Stay`, `Outer Forestay`, `Bowsprit`.
   - Кнопка **`Active Sail Plan`** открывает конфигуратор с выпадающими списками (фильтруются по инвентарю L1) и 5 макро-пресетами смены парусов:
     - `⛵ Cruise`: Грот + Генуя (100% круиз).
     - `⚡ Race / Code 0`: Грот + Код 0 (мощность на слабых ветрах).
     - `🎈 Downwind`: Грот + Геннакер A2 (полные курсы).
     - `⛈ Storm`: Штормовой стаксель solo (штормование).
     - `⚙ Bare Poles`: 0% парусов (мотор / дрейф).
   - Текущий уровень рифа отображается бейджем статуса (`[R1 75%]`, `[R2 50%]`, `[R3 35%]`) рядом с гротом.
3. **L3: Физический такелаж и рифление (Physical Rig & Reefing Dock)**:
   - В центральной колонке кокпита находится **Reefing Dock** (`FULL MAIN`, `REEF 1`, `REEF 2`, `REEF 3`) для оперативного взятия рифов на лету.
   - Физические карточки 12 канатов (фалы, риф-штерты, шкоты, оттяжка) обеспечивают плавную набивку и блокировку стопорами `🔒 CLAMPED`.

### Возможности пульта:
1. **Центральная колонка штурвала и аксиометра (Rotary Helm & Rudder)**:
   - Вращающийся морской штурвал с рукояткой центрального пера (**King Spoke**) и индикатором оборотов ($\pm 2.0$ об, диапазон перекладки пера $\pm 35.0^\circ$).
   - Интерактивное кручение мышью (**Drag / Pointer Capture**) и колесом мыши с плавным доруливанием на 60 FPS.
   - Двойной клик (**Double Click**) или нажатие на ступицу мгновенно центрирует штурвал в диаметральную плоскость (ДП $0.0^\circ$).
   - 2-стрелочный аналоговый аксиометр: отображение фактического угла пера (**Actual**) и команды автопилота/SIA (**Command**).
2. **Интерактивные карточки канатов и фалов**:
   - **Dual-Bar индикация**: верхняя полоса — процент набивки (**TRIM % / Длина в метрах**), нижняя полоса — динамическая нагрузка (**LOAD N / SWL N**) с цветовой индикацией безопасной нагрузки (зеленый $\to$ желтый $\to$ красный $\to$ перегруз).
   - Интерактивное плавное натяжение мышью (**Drag & Drop**) и колесом мыши прямо над карточкой каната.
   - Учет галса судна (**Tack Awareness**): автоматическое затемнение и ослабление шкота подветренной/наветренной стороны в зависимости от $AWA$.
3. **Морские клавишные стопоры (Clutch Stoppers & Hardware Guard)**:
   - Статус зажатия стопора: `🔒 CLAMPED` (темно-синий индикатор) / `🔓 EASED` (изумрудно-зеленый пульсирующий индикатор).
   - **Строгая блокировка**: при закрытом стопоре карточка каната физически заблокирована от случайного сдвига (курсор `not-allowed`, штриховка безопасности). Попытка потянуть зажатый канат вызывает визуальный импульс красной тревоги (**Flash Alert**) и предупреждение в журнал.
4. **Погон гика (Traveler) и тормоз (Brake)**:
   - Плавный ползунок перемещения каретки погона на левый/правый борт ($[-100\%\dots +100\%]$) с независимым тормозом `🔒 BRAKE` / `🔓 FREE`.
5. **Мгновенные пресеты рифления (Reef Presets)**:
   - Кнопки быстрого взятия рифов: `FULL MAIN`, `REEF 1`, `REEF 2` с автоматической набивкой риф-штертов и согласованным потравливанием шкота.
6. **Режимы управления (Dual Control Modes)**:
   - `🤖 АВТОПИЛОТ`: рулевое устройство и шкоты управляются алгоритмами сценария.
   - `🕹 ШКИПЕР (РУЧНОЕ)`: прямое ручное вмешательство капитана. Касание любого инструмента автоматически переводит симуляцию в ручной режим без сброса физического времени.

---

## 🗗 Автономный мостик на отдельном мониторе (Standalone Pop-out Bridge)

Для удобства шкипера и работы на 2-мониторных конфигурациях реализован выделенный интерфейс мостика (**`/cockpit.html`**):

- **Кнопка выноса `[🗗 В окне]`**: мгновенно открывает изолированное окно с кокпитом, адаптированное под сенсорные экраны и планшеты.
- **Полноэкранный режим (`⛶ Fullscreen`)**: скрывает элементы браузера для полного погружения.
- **Zero-Latency 60 FPS Mirror Sync**: мгновенная двусторонняя синхронизация между главным экраном и дочерним окном через `BroadcastChannel('sia_workbench_sync')` без сетевых задержек.
- **REST API интерфейс**:
  - `GET /api/v1/telemetry/rig` — получение текущей телеметрии нагрузок и положений рангоута.
  - `POST /api/v1/controls/rig` — пакетная отправка управляющих команд шкипера (натяжение, стопоры).
  - `POST /api/v1/controls/preset` — выполнение пресетов рифления.
...
Проект покрыт **263 автоматизированными тестами**:
...
* `test_sails_ropes_contracts.py` — Валидация контрактов такелажа, канатов, погона гика и закрутки парусов.
* `test_rig_physics.py` — Физические тесты 12 канатов, динамических нагрузок, стопоров и интерлоков безопасности.
* `test_rig_api.py` — Тестирование REST API эндпоинтов телеметрии и управления рангоутом (`/api/v1/telemetry/rig`, `/api/v1/controls/rig`).
...
* `test_workbench_server.py` — Верификация эндпоинтов `/api/scenarios`, `/api/run`, `/api/sails`, `/api/query-action`.
* `test_workbench_integration.py` — Проверка раздачи статики, автономного мостика (`/cockpit.html`) и веб-модулей.
...
# Запуск полного набора тестов (263 passed)
uv run pytest
```
