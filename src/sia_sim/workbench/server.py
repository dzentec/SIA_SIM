...
def format_run_payload(runner_result: Any) -> dict[str, Any]:
    ...
    for r in records:
        ...
        mainsheet = (
            round(sf.actuators.mainsheet_pct, 1)
            if sf.actuators.mainsheet_pct is not None
            else None
        )

        sel_resp = (
            dec.selected_response.model_dump(mode="json")
            if dec.selected_response
            else None
        )
...
class WorkbenchRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_get_scenarios(self) -> None:
        scenarios = [
            {
                "id": "sim005",
                "name": "SIM-005 Broach Precursor (18 kt Gust + 12 kN Wave)",
                "description": "Beam reach encountering quartering wave impact and wind shift.",
                "duration_ms": 20000,
            },
            {
                "id": "benign",
    ...
    def _handle_run_simulation(self, payload: dict[str, Any]) -> None:
        scenario_name = payload.get("scenario", "sim005")
    ...
    def _handle_query_action(self, payload: dict[str, Any]) -> None:
        ...
        if sail_set in ("REEF_1", "REEF_2"):
            ...
            note = (
                f"Skipper confirmed {sail_set} — minimal mainsail drive. "
