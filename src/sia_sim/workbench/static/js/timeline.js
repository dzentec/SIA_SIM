...
const TimelineRenderer = {
  ...
  bindEvents() {
    ...
    const findEventAtPos = (clientX, clientY) => {
      ...
      // Event blocks are rendered across the event track (Y: 18px to 62px)
      if (canvasY < 18 || canvasY > 62) {
        return null;
      }
  ...
  renderTracks() {
    ...
    // Event Track Background
    ctx.fillStyle = 'rgba(255, 255, 255, 0.02)';
    ctx.fillRect(0, 18, w, h - 18);

    // Time Ruler Horizontal Divider
    ...
    for (const evt of this.events) {
      ...
      ctx.beginPath();
      if (ctx.roundRect) {
        ctx.roundRect(x1, trackY, blockW, trackH, 5);
      } else {
        ctx.rect(x1, trackY, blockW, trackH);
      }
      ctx.fill();
      ctx.stroke();
      ...
      ctx.fillRect(x1 + 2, trackY + 4, 2, trackH - 8);
      ctx.fillRect(x1 + blockW - 4, trackY + 4, 2, trackH - 8);
      ...
      ctx.fillText(label, x1 + 8, trackY + trackH / 2);
    }
  },
};
